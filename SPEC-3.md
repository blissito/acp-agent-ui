...
> Día 3 del POC. La app vive en [`web3/`](web3/), junto a la `web/` del SPEC-2, que se queda
> intacta como referencia verificada.
>
...
De `block/goose`, `ui/desktop` (Apache-2.0 — la atribución está en [`web3/NOTICE`](web3/NOTICE)).
...
```sh
cd acp-ghosty-poc/web3
npm install
node --env-file=.env node_modules/.bin/react-router dev    # Vite + SSR

npm run build && node --env-file=.env server.js            # producción, Express
```

El `.env` (modo 600, fuera del repo) lleva `ACP_WS_URL`, `ACP_SECRET`, `ACP_CWD` y `AGENT_BOX_ID`.
