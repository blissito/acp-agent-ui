...
## 3. Backend (`web/server.mjs`, Node puro `node:http`)
...
## 4. Frontend (`web/client/` — Vite + React Router v7 + Streamdown) — **sin cambios**
...
cd acp-ghosty-poc/web/client && npm install && npm run build

# 2) un solo server (SPA + API + SSE) en :4000
cd acp-ghosty-poc/web
PORT=4000 \
...
> El camino correcto (ya aplicado en `web/client`) es **oficial**:
>
